...
while True:
    ...
    if len(idxs) > 0:
        ...
        for i in idf:
            ...
            cv2.putText(FR, "Analisis de distanciamiento social COVID-19", (210, H+60),
                        cv2.FONT_HERSHEY_DUPLEX, 1, (0, 0, 0), 2)
            cv2.rectangle(FR, (20, H+80), (510, H+180), (100, 100, 100), 2)
            cv2.putText(FR, "Lineas muestran proximidad entre las personas.", (30, H+100),
                        cv2.FONT_HERSHEY_DUPLEX, 0.6, (100, 100, 0), 2)
            cv2.putText(FR, "-- Amarillo: Cercano", (50, H+90+40),
                        cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 170, 170), 2)
            cv2.putText(FR, "-- Rojo: Muy Cercano", (50, H+40+110),
                        cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 0, 255), 2)
            # cv2.putText(frame, "--    PINK: Pathway for Calibration", (50, 150),
            #             cv2.FONT_HERSHEY_DUPLEX, 0.5, (180,105,255), 1)

            cv2.rectangle(FR, (535, H+80), (1100, H+140+40), (100, 100, 100), 2)
            cv2.putText(FR, "Rectangulos muestran el nivel de riesgo de la persona.", (545, H+100),
                        cv2.FONT_HERSHEY_DUPLEX, 0.6, (100, 100, 0), 2)
            cv2.putText(FR, "-- Rojo: Riesgo Alto", (565, H+90+40),
                        cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 0, 150), 2)
            cv2.putText(FR, "-- Naranjo: Riesgo Bajo", (565, H+150),
                        cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 120, 255), 2)

            cv2.putText(FR, "-- Verde: Sin Riesgo", (565, H+170),
                        cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 150, 0), 2)


            if frameCounter % 10 == 0:
                tot_str = "N personas: " + str(total_p)
                high_str = "N Riesgo Alto: " + str(high_risk_p)
                low_str = "N Riesgo Bajo: " + str(low_risk_p)
                safe_str = "N Sin Riesgo: " + str(safe_p)
            ...
            cv2.putText(FR, low_str, (400, H +25),
                        cv2.FONT_HERSHEY_DUPLEX, 0.6, (0, 120, 255), 2)
            cv2.putText(FR, high_str, (600, H +25),
                        cv2.FONT_HERSHEY_DUPLEX, 0.6, (0, 0, 150), 2)
    ...
    if writer is None:
        ...
        writer = cv2.VideoWriter("results/covid19_"+vname, fourcc, 24,
                                 (frame.shape[1], frame.shape[0]), True)

    writer.write(frame)
print("Processing finished: open"+"covid19_"+vname)
writer.release()
vs.release()
